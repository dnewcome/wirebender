...
def ensure_meshes(convert=True):
    ...
    Returns (world, cfgs, mesh_assets). Set convert=False to skip the STL
    conversion when the meshes already exist (e.g. for fast per-frame rebuilds).
    """
...
def build_model_xml(world, cfgs, mesh_assets, extra_world="",
                    with_wire=True, with_actuators=True):
    """Build the machine MJCF string.

    extra_world      : raw XML injected into <worldbody> (e.g. a formed-wire body)
    with_wire        : include the straight wire body + feed slide joint
    with_actuators   : include the <actuator> block (position servos)
    """
    ...
    </body>""" if with_wire else ""

    feed_act = ('    <position name="feed"     joint="feed"     '
                f'ctrlrange="{FEED_RANGE_MM[0]*MM_TO_M:.4g} {FEED_RANGE_MM[1]*MM_TO_M:.4g}" kp="200"/>\n'
                ) if with_wire else ""
    actuator_block = f"""
  <actuator>
{feed_act}    <position name="tube_rot" joint="tube_rot" ctrlrange="{TUBE_RANGE[0]} {TUBE_RANGE[1]}"/>
    <position name="bend"     joint="bend"     ctrlrange="{BEND_RANGE[0]} {BEND_RANGE[1]}"/>
  </actuator>""" if with_actuators else ""
...
def main():
    world, cfgs, mesh_assets = ensure_meshes(convert=True)
    XML_OUT.write_text(build_model_xml(world, cfgs, mesh_assets))
